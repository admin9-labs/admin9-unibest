---
name: wot-ui-v2
description: '回答、生成、重构、排查 wot-ui v2 相关代码时使用。关键词：wot-ui、uni-app、Vue3、wd-、ConfigProvider、useToast、useDialog、Form、Popup、theme。适用于组件选型、API 查询、示例页面生成、主题定制、常见坑排查。'
argument-hint: '组件名、页面场景、问题描述或主题定制需求'
---

# Wot UI V2 Skill

这个 skill 用于让 Agent 在处理 wot-ui v2 相关任务时，优先采用组件库既有能力、遵守 uni-app 场景限制，并通过本项目的 `pnpm wot` script 查询离线知识。
...
1. 先用本项目的 `pnpm wot` 查询组件知识；MCP 仅作为可选入口。
...
1. `pnpm wot list` 找组件名。
2. `pnpm wot info <Component>` 看 props、events、slots、CSS 变量。
3. `pnpm wot demo <Component>` 看 demo 名称或具体 demo 代码。
4. `pnpm wot doc <Component>` 看完整 markdown 文档。
5. `pnpm wot token <Component>` 看主题变量。
