...
本文件提供适合本项目生成代码与回答问题的 wot-ui v2 高价值知识，不复制整份上游文档或编辑器规则。
...
- 本项目采用 npm 安装模式，组合式函数、类型和工具从 `@wot-ui/ui` 导入。
- 组件自动导入沿用现有 `WotResolver`/easycom 配置，并由 resolver 生成 `src/types/components.d.ts`。
- 禁止在 `tsconfig.json` 中加入 `@wot-ui/ui/global`。Wot UI 2.2.0 的该入口会把源 SFC 类型错误带入本项目。

## Import Rules

- 组合式函数、类型和工具统一从 `@wot-ui/ui` 导入。
- 官方文档示例若使用 `@/uni_modules/wot-ui`，在本项目中应转换为 `@wot-ui/ui`。
- 不要绕过现有 resolver 重复配置组件全局类型。
...
- 在本项目中，按 `pnpm wot list` -> `pnpm wot info` -> `pnpm wot demo` -> `pnpm wot doc` -> `pnpm wot token` 的顺序获取组件知识。
- 修改项目后运行 `pnpm wot:doctor`、`pnpm wot:usage` 和 `pnpm wot:lint`，或使用聚合命令 `pnpm wot:check`。
- 不要在本应用中执行上游 CLI 仓库的数据提取、源码调试或构建产物调试流程。
- MCP 不是启动或开发本应用的前置条件；本地 CLI/MCP 依赖缺失时，继续依据项目代码和本参考完成任务，并在结果中说明未运行的检查。
