...
H5 首版既定游客端能力与 AI 聊天式真流式升级均已形成真实 API、OpenAPI、H5 页面、隔离测试和移动端浏览器闭环；当前无未完成首版切片。后续仅在用户提出新的已定义产品能力或缺陷时启动维护切片：先在 API Base URL `http://travel.wifixc.test/api` 上核对公开契约，再在 H5 代理 target `http://travel.wifixc.test` 下完成对应页面、测试和 ego lite 验收。不得操作 MAMP、切换到历史 API 域名或把 API 主机当作 H5 页面入口。
