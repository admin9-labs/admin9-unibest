...
describe('map point adapter', () => {
  it('combines valid real content coordinates and tolerates one failed source', async () => {
    sources.getAttractions.mockResolvedValue([{ code: 'lake', name: '邛海', address: '海滨路', latitude: 27.86, longitude: 102.27 }])
    sources.getScenicSpots.mockResolvedValue([{ code: 'invalid', name: '无坐标', latitude: null, longitude: null }])
    sources.getRestaurants.mockRejectedValue(new Error('network'))
    sources.getAccommodations.mockResolvedValue([{ code: 'hotel', name: '湖畔酒店', latitude: 27.87, longitude: 102.28 }])

    const points = await getMapPoints()

    expect(points).toHaveLength(2)
    expect(points[0]).toMatchObject({ id: 'attraction:lake', typeName: '景区', detailUrl: '/pages/attractions/detail?code=lake' })
    expect(points[1]).toMatchObject({ id: 'accommodation:hotel', address: null })
  })
