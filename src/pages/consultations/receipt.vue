...
<template>
  <view class="page">
    <view v-if="ticket && credential" class="receipt">
      <wd-icon name="check-circle" size="54" color="#23744f" /><view
        class="title"
      >
        咨询已提交
      ...
      </view><wd-button block size="large" @click="query">
        查看办理进度
      </wd-button>
    </view><view v-else class="missing">
      <wd-empty tip="当前设备未找到该工单的查询凭证" /><wd-button
        block
        @click="manualQuery"
      >
        手动输入凭证查询
