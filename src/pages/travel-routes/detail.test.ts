...
describe('travel route detail page', () => {
  it('loads ordered nodes and opens the matching content URL', async () => {
    getTravelRoute.mockResolvedValueOnce({ code: 'lake-day', name: '邛海一日游', summary: null, description: null, duration_minutes: 480, cover: null, nodes: [{ node_type: 'scenic_spot', position: 1, stay_minutes: 120, note: '上午游览', target: { code: 'wetland', name: '邛海湿地' } }] })
    const wrapper = mount(TravelRouteDetail, { global: { stubs: { WdButton: true, WdLoading: true, WdEmpty: true, WdImg: true, WdIcon: true } } })
