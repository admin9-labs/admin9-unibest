...
describe('aI assistant list page', () => {
  it('opens an enabled assistant at its independent URL', async () => {
    getAiAssistants.mockResolvedValueOnce([{ id: 901, name: '西昌文旅助手', description: '游览问答', welcome_message: '您好' }])
    ...
    expect(uni.navigateTo).toHaveBeenCalledWith({ url: '/pages/ai-assistants/chat?id=901' })
  })
})
