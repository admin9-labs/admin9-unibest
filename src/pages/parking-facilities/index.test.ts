...
describe('parking facility list', () => {
  it('lists only API records and supports the two approved types', async () => {
    getParkingFacilities
      .mockResolvedValueOnce({ records: [{ id: 1, name: '建昌古城外围停车场', type: 'parking_lot', summary: '外围停车后步行入城', address: '府街', cover: { url: '/parking-cover.jpg' }, opening_hours: '以现场公示为准', total_spaces: 120 }], meta: { page: 1, has_more: true } })
    ...
    const wrapper = mount(ParkingFacilityList, { global: { stubs: { WdSearch: true, WdTag, WdImg: { props: ['src'], template: '<img :src="src">' }, WdIcon: true, WdLoading: true, WdEmpty: true, WdButton, ScrollView: { template: '<div><slot /></div>' } } } })
    await vi.mocked(onLoad).mock.calls.at(-1)?.[0]?.()
    await flushPromises()
    expect(wrapper.text()).toContain('建昌古城外围停车场')
    expect(wrapper.text()).toContain('总车位 120')
    expect(wrapper.get('img').attributes('src')).toBe('/parking-cover.jpg')
  ...
  it('ignores an old page response after a newer type filter starts', async () => {
    ...
    const wrapper = mount(ParkingFacilityList, { global: { stubs: { WdSearch: true, WdTag, WdImg: true, WdIcon: true, WdLoading: true, WdEmpty: true, WdButton, ScrollView: { template: '<div><slot /></div>' } } } })
    await vi.mocked(onLoad).mock.calls.at(-1)?.[0]?.()
    await flushPromises()
